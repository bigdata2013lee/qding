...
class XingeHelper(object):
    ...
    def Request(cls, path, params):
        ...
        if cls.HTTP_METHOD == 'GET':
            httpClient.request(cls.HTTP_METHOD, ('%s?%s' % (path, urllib.parse.urlencode(params))), headers=cls.HTTP_HEADERS)
        ...
        if 200 != response.status:
            ...
            data = response.read()
            retDict = json.loads(data.decode('utf8'))
...
def _BuildAndroidMsg(title, content):
    msg = Message()
...
def PushAccountAndroid(accessId, secretKey, title, content, account):
    x = XingeApp(accessId, secretKey)
    return x.PushSingleAccount(0, account, _BuildAndroidMsg(title, content))

def PushAllAndroid(accessId, secretKey, title, content):
    x = XingeApp(accessId, secretKey)
    return x.PushAllDevices(0, _BuildAndroidMsg(title, content))

def PushTagAndroid(accessId, secretKey, title, content, tag):
    x = XingeApp(accessId, secretKey)
    return x.PushTags(0, (tag,), 'OR', _BuildAndroidMsg(title, content))
